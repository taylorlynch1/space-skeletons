...
- Enemy roster: Slasher (wave 1+), Gunner (wave 2+), Brute (wave 6+,
  ...
  Golden Skeleton (every other non-boss wave from wave 2, darts and
  flees after 8s, 100 pts, blast-immune, never attacks).
...
3. DONE 2026-06-12, revised same day after iPad playtest. Shield is
   an active ability: pickups arm one stored charge (max one, extras
   pay +150), the bubble button left of SUPER BLAST starts a 6s
   invincibility bubble blocking everything including melee, combo
   protected, draining ring with a last-1.5s warning blink, activate
   and expire sounds. Tunables in src/shield.js. Same pass: aim
   assist reworked to world-space radius capture with stickiness
   (map 6.15), pickup spawns staggered, spaced, and shrunk to 70
...
4. DONE 2026-06-12. Golden Skeleton: glowing gold darting flier with
   shimmer aura and sparkle trail, strictly every other non-boss wave
   from wave 2 (2, 4, 7, 9...), max one, arrives mid-wave with a
   twinkle and banner, flees after 8s with a doppler whistle and a
   TOO SLOW! pop. 100 pts multiplied by combo, big gold score pop,
   ka-ching over the bonk. Blast-immune: a SUPER BLAST scares him off
   instead (map 6.13 resolved). Splash can still kill him. Tunables:
   GOLDEN_WINDOW and hp in skeletons.js.
